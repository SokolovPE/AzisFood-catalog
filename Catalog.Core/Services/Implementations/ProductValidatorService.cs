...
public class ProductValidatorService : IValidatorService<Product>
{
    ...
    {
        if (item.ServingSize <= 0) return new Tuple<bool, string>(false, "Serving size must be greater than zero");

        var items = await _ingredientService.GetAllAsync();
        if (items.Count == 0) return new Tuple<bool, string>(false, "Cannot fetch ingredients");

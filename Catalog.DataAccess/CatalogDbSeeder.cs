...
public class CatalogDbSeeder
{
    ...
    private void SeedCategories()
    {
        ...
        if(changes != categorySeed.Length)
            _logger.LogWarning($"[Categories] Seed length {categorySeed.Length} is not equal to added row count: {changes}");
    }
    ...
    private void SeedIngredients()
    {
        ...
        if(changes != ingredientSeed.Length)
            _logger.LogWarning($"[Ingredients] Seed length {ingredientSeed.Length} is not equal to added row count: {changes}");
    }
